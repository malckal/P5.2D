...
p5.play
by Paolo Pedercinimolleindustria, 2015
httpmolleindustria.org
...
factory(root.p5);
 ...
 @see httpsgithub.commolleindustriap5.playissues46
p5.prototype.registerMethod('init', function p5PlayInit() {
 ...
 httpsgithub.comprocessingp5.jsissues1263
...
p5.prototype.updateSprites = function(upd) {
  ...
  for(var i = 0; ithis.allSprites.size(); i++)
  {
...
p5.prototype.getSprites = function() {
  ...
  else
  {
    var arr = [];
    for every tag
    for(var j=0; jarguments.length; j++)
    {
      for(var i = 0; ithis.allSprites.size(); i++)
      {
...
var KEY_WENT_UP = 3;
 ...
 @method keyWentDown
 @param {NumberString} key Key code or character
...
p5.prototype.KEY_DEPRECATIONS = {
  ...
  @return {numberundefined} a numeric JavaScript key code, or undefined
...
deltaTime = ((now - then)  1000)INTERVAL_60;  seconds since last frame
    ...
    {{#crossLink p5.playcreateSpritemethod}}{{crossLink}}.
 ...
 httpsgithub.commolleindustriap5.playpull67
...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  this.colliderType = 'none';
   ...
   Object containing information about the most recent collisionoverlapping
  ...
  this.getDirection = function() {
    ...
    if(isNaN(direction))
     ...
     See httpsgithub.commolleindustriap5.playissues94
    if (pInst._angleMode === pInst.RADIANS) {
  ...
  this.attractionPoint = function(magnitude, pointX, pointY) {
   ...
   @method addImage
   @param {Stringp5.Image} label Label or image
  ...
  this.addAnimation = function(label)
  {
   ...
   Changes the displayed imageanimation.
   Equivalent to changeAnimation
  
   @method changeImage
   @param {String} label ImageAnimation identifier
  ...
  this.changeImage = function(label) {
   ...
   @return {String} label ImageAnimation identifier
  ...
  this.overlapPixel = function(pointX, pointY) {
    ...
    point.x -= this.position.x-img.width2;
    point.y -= this.position.y-img.height2;

    out of the image entirely
    if(point.x0  point.ximg.width  point.y0  point.yimg.height)
      return false;
  ...
  this.overlapPoint = function(pointX, pointY) {
   ...
   they will be created automatically from the imageanimation bounding box.
      ...
      if(this !== others[i] && !this.removed) you can check collisions within the same group but not on itself
      {
        ...
        if(type === 'overlap') {
        ...
        else if(type === 'collide'  type === 'displace'  type === 'bounce')
          {
            ...
            var tunnelX = abs(this.velocity.x-other.velocity.x) = other.collider.extents.x2 && round(this.deltaX - this.velocity.x) === 0;

            var tunnelY = abs(this.velocity.y-other.velocity.y) = other.collider.size().y2 && round(this.deltaY - this.velocity.y) === 0;
            ...
            if(displacement.x !== 0  displacement.y !== 0)
            {
              ...
              if(type === 'bounce')
              {
                ...
                if(abs(displacement.x)abs(displacement.y))
                {
                  ...
                  if(!this.immovable)
                  {
                    this.velocity.x = newVelX1this.restitution;

                  }

                  if(!other.immovable)
                    other.velocity.x = newVelX2other.restitution;
                ...
                if(abs(displacement.x)abs(displacement.y))
                {
                  ...
                  if(!this.immovable)
                    this.velocity.y = newVelY1this.restitution;

                  if(!other.immovable)
                    other.velocity.y = newVelY2other.restitution;
                }
                }
              }
...
function cameraPush() {
  ...
  if(!camera.init && camera.position.x === 0 && camera.position.y === 0)
    {
    camera.position.x=pInst.width2;
    camera.position.y=pInst.height2;
    camera.init = true;
    }

  camera.mouseX = pInst.mouseX+camera.position.x-pInst.width2;
  camera.mouseY = pInst.mouseY+camera.position.y-pInst.height2;
  ...
  if(!camera.active)
  {
    ...
    pInst.translate(-camera.position.x+pInst.width2camera.zoom, -camera.position.y+pInst.height2camera.zoom);
  }
}
...
function Group() {
  ...
  function virtEquals(obj, other) {
    ...
    @return {boolean} True if any collisionoverlap occurred
...
function CircleCollider(pInst, _center, _radius, _offset) {
  ...
  if(_offset === undefined)
    ...
    this.offset = _offset;
  this.extents = createVector(_radius2, _radius2);
  ...
  this.draw = function()
  {
    ...
    pInst.ellipse(this.center.x+this.offset.x, this.center.y+this.offset.y, this.radius2, this.radius2);
  };
  ...
  this.size = function()
  {
    return createVector(this.radius2, this.radius2);
  };
...
function AABB(pInst, _center, _extents, _offset) {
  ...
  this.right = function()
  {
    return this.center.x+this.offset.x + this.extents.x2;
  };
  ...
  this.top = function()
  {
    return this.center.y+this.offset.y - this.extents.y2;
  };
  ...
  this.draw = function()
  {
    ...
    pInst.stroke(0, 255, 0);
    pInst.rectMode(CENTER);
    pInst.rect(this.center.x+this.offset.x, this.center.y+this.offset.y, this.size().x2, this.size().y2);
  };
      ...
      if(distanceother.radius)
      {
        ...
        if(pt.x === other.center.x && pt.y === other.center.y)
        {
          ...
          if(a === 0)
          {
            ...
            if(pt.y === this.top()) a = PI2;
            if(pt.y === this.bottom()) a = -PI2;
          }
        }
        else
        {
...
defineLazyP5Property('AABB', boundConstructorFactory(AABB));
  ...
  datafile0001.png and datafile0005.png the images
  datafile0003.png and datafile0004.png will be loaded as well.
  ...
  @example
      ...
      function preload() {
        sequenceAnimation = loadAnimation(datawalking0001.png, datawalking0005.png);
        glitch = loadAnimation(datadog.png, datahorse.png, datacat.png, datasnake.png);
      }
...
function Animation(pInst) {
  ...
  if(frameArguments.length === 2 && typeof frameArguments[0] === 'string' && typeof frameArguments[1] === 'string')
  {
    ...
    if(from !== -1 && to !== -1)
    {
      ...
      var prefix2 = to.substring(0, to.length-(4+digits2) );
       ...
       httpstackoverflow.coma87634272422398.
  ...
  this.update = function() {
    ...
    if ( this.playing && cycles%this.frameDelay === 0)
    {
      ...
      if(targetFrameframe && targetFrame !== -1)
      {
        frame++;
      }
      going to taget frame down
      else if(targetFrameframe && targetFrame !== -1)
      {
        frame--;
      }
...
defineLazyP5Property('Animation', boundConstructorFactory(Animation));
  ...
  @example
      ...
      explode_sprite_sheet = loadSpriteSheet('assetsexplode_sprite_sheet.png', 171, 158, 11);
 
       Method 2 - Using an array of objects that define each frame
      var player_frames = loadJSON('assetstiles.json');
      player_sprite_sheet = loadSpriteSheet('assetsplayer_spritesheet.png', player_frames);
...
function construct(constructor, args) {
  ...
  Javascript Quadtree
  based on
  httpsgithub.comtimohausmannquadtree-js
...
Quadtree.prototype.retrieveFromGroup = function( pRect, group ) {
  ...
  for(var i=0; icandidates.length; i++)
